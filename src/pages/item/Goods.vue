    ...
    >
      <template slot="items" slot-scope="props">
        ...
        <td class="justify-center layout px-0">
          ...
          <v-btn icon v-if="props.item.saleable" @click="updateSaleable(props.item.id,false)">下架</v-btn>
          <v-btn icon v-else @click="updateSaleable(props.item.id,true)">上架</v-btn>
        </td>
      </template>
    </v-data-table>
    <!--弹出的对话框-->
  ...
  export default {
    ...
    methods: {
      ...
      updateSaleable(id, saleable) {
        this.$message.confirm("确认要" + (saleable ? '上架' : '下架') + "商品吗？")
          .then(() => {
            this.$http.put("/item/goods/" + id + "/" + saleable)
              .then(() => {
                this.getDataFromServer();
              })
              .catch(() => {
                this.getDataFromServer();
                this.$message.error((saleable ? '上架' : '下架') + "失败！");
              })
          })
          .catch(() => {
            this.getDataFromServer();
